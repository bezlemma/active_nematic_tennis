...
const DT = 0.001f0 # Time step for Q evolution and fluid solver
...
const λ = 0.7f0  # Flow alignment parameter λ
const γ = 1.0f0  # Rotational/Relaxation viscosity γ
const α = 0.6f0  # Activity strength α (>0 for extensile)
const LANDAU_DEGENNES_A = 1.0f0
const LANDAU_DEGENNES_C = 1.0f0
const FRANK_ELASTIC_K = 0.01f0

# --- Paddle/Game Constants ---
...
function set_boundary_velocity!(b::Int, x::Vector{Float32})
    ...
    # 'b' determines if 'x' is vx (b=1) or vy (b=2).
...
function calculate_H!(Hxx_out::Vector{Float32}, Hxy_out::Vector{Float32},
    ...
    # Calculates molecular field H_ij = - (δF / δQ_ij), from Landau-de Gennes free energy F.
    # F = ∫ [ A/2 Tr(Q²) + C/4 (Tr(Q²))² + K/2 |∇Q|² ] dV
    # This code uses H_ij = - (A Q_ij + 2C Tr(Q²) Q_ij - K ∇²Q_ij) based on specific model conventions.
    Threads.@threads for j in 2:(NY+1)
        for i in 2:(NX+1)
            ...
            # For traceless Q in 2D: Tr(Q²) = 2(Q_xx² + Q_xy²)
...
function calculate_S_flow_term!(Sxx_out::Vector{Float32}, Sxy_out::Vector{Float32},
    ...
    # Calculates S_ij = λ D_ij - [Ω, Q]_ij, the flow-coupling term in Q-tensor evolution.
    # D_ij: strain rate tensor. Ω_ij: vorticity tensor. [A,B]: commutator.
    Threads.@threads for j in 2:(NY+1)
        for i in 2:(NX+1)
            idx = IX(i,j)
...
function calculate_div_active_stress!(div_sx::Vector{Float32}, div_sy::Vector{Float32},
    ...
    # Calculates divergence of active stress: F^A = ∇ ⋅ σ^A, where σ_ij^A = -α Q_ij.
    Threads.@threads for j in 2:(NY+1)
        for i in 2:(NX+1)
            idx = IX(i,j)
            ...
            div_sy[idx] = -alpha_activity * (dQxy_dx - dQxx_dy) # Uses Q_yy = -Q_xx
        end
    end
end
...
function calculate_div_elastic_stress!(div_out_x::Vector{Float32}, div_out_y::Vector{Float32},
    ...
    # Calculates divergence of elastic stress F^E = ∇ ⋅ σ^E.
    # σ_ij^E = -λ [Q,H]_ij - K (∂Q_lm/∂x_i)(∂Q_lm/∂x_j) (sum over l,m)
    ...
    Threads.@threads for j_idx in 2:(NY+1) # Terms involving H
        for i_idx in 2:(NX+1)
            ...
            comm_QH_xx_val = 0.0f0 # [Q,H]_xx for 2D symmetric traceless tensors
            comm_QH_xy_val = 2.0f0 * (q_xx * h_xy - q_xy * h_xx) # [Q,H]_xy

            sigma_el_xx_temp[idx] = -lambda_align * comm_QH_xx_val
            sigma_el_xy_temp[idx] = -lambda_align * comm_QH_xy_val
            sigma_el_yy_temp[idx] = lambda_align * comm_QH_xx_val # Uses [Q,H]_yy = -[Q,H]_xx
        end
    end

    Threads.@threads for j_idx in 2:(NY+1) # Terms involving K_elastic
        for i_idx in 2:(NX+1)
            idx = IX(i_idx, j_idx)
...
function fluid_step!(p1_is_pushing::Bool, p1_y_world::Float32, p2_is_pushing::Bool, p2_y_world::Float32)
    ...
    calculate_H!(fluid_Hxx, fluid_Hxy, fluid_Qxx, fluid_Qxy,
                 FRANK_ELASTIC_K, LANDAU_DEGENNES_A, LANDAU_DEGENNES_C)
    ...
    calculate_H!(fluid_Hxx, fluid_Hxy, fluid_Qxx, fluid_Qxy, # Recalculate H with updated Q
                  FRANK_ELASTIC_K, LANDAU_DEGENNES_A, LANDAU_DEGENNES_C)

    calculate_div_active_stress!(fluid_div_sigma_active_x, fluid_div_sigma_active_y, fluid_Qxx, fluid_Qxy, α)
    calculate_div_elastic_stress!(fluid_div_sigma_elastic_x, fluid_div_sigma_elastic_y,
                                  fluid_Qxx, fluid_Qxy, fluid_Hxx, fluid_Hxy, FRANK_ELASTIC_K, λ)

    Threads.@threads for i in 1:FLUID_SIZE # Add Green Color Based on Nematic Activity
        ...
        if active_force_sq_mag > ACTIVITY_GREEN_DIV_STRESS_SQ_THRESHOLD
            fluid_color_G[i] = min(1.0f0, fluid_color_G[i] + ACTIVITY_GREEN_SCALAR_ADD * DT) # Apply as a rate
        end
    end

    Threads.@threads for i in 1:FLUID_SIZE # Sum Forces
    ...
    # Solve for velocity: (0 - μ∇²)v = F_total
    general_linear_solve!(1, fluid_vx, fluid_vx0, 0.0f0, μ * INV_DX2, μ * INV_DY2, SOLVER_ITER)
    general_linear_solve!(2, fluid_vy, fluid_vy0, 0.0f0, μ * INV_DX2, μ * INV_DY2, SOLVER_ITER)

    project!(fluid_vx, fluid_vy, fluid_p, fluid_div) # Enforce incompressibility
end
...
function apply_paddle_jet_color_effect!(
    ...
    # director_angle = (paddle_id == 1) ? 0.0f0 : Float32(pi) # Unused as Q-setting is commented out
...
function run()
    ...
    fluid_x_range_vis = LinRange(0.0f0 + FLUID_DX/2.0f0, COURT_WIDTH - FLUID_DX/2.0f0, NX); # LinRange of Float32
    fluid_y_range_vis = LinRange(0.0f0 + FLUID_DY/2.0f0, COURT_HEIGHT - FLUID_DY/2.0f0, NY); # LinRange of Float32
    ...
    function update_visualization_data!()
        ...
        for j_glob in 1:ARROW_SUBSAMPLE:NY, i_glob in 1:ARROW_SUBSAMPLE:NX
            ...
            # Director angle φ from Q_xx ~ S/2 cos(2φ), Q_xy ~ S/2 sin(2φ)
            # So, 2φ = atan(Q_xy, Q_xx)
            angle_director = 0.5f0 * atan(qxy_val, qxx_val) # atan returns Float32 if args are Float32
            director_x = cos(angle_director) # cos(Float32) is Float32
            director_y = sin(angle_director) # sin(Float32) is Float32
                
            norm_dir = sqrt(director_x^2 + director_y^2) # Float32
            if norm_dir > Float32(epsilon_director) # Compare Float32 with Float32(Float64)
    ...
    on(events(fig).tick) do _
        ...
        current_time = time()
        dt_game = Float32(clamp(current_time - last_update_time[], 0.001, 0.05)) # dt_game is Float32
        ...
        if Keyboard.a in keys && serve_state[] == :playing 
            ...
            dir_to_paddle = paddle_center_world - bp; dist_sq = sum(dir_to_paddle .^ 2) # dist_sq is Float32
            if dist_sq > 1e-4; direct_pull_force += normalize(dir_to_paddle) * PULL_FORCE_STRENGTH; end # 1e-4 is Float64
        end
        ...
        if serve_state[] == :playing
            ...
            cbs_val_local = cbs_val # Use local copy for modification
            if new_bv[1]<0.0f0 && rect_overlaps(paddle_left_rect, ball_rect_predict)
                 paddle_center_y_val = p_left_y_val+PADDLE_HEIGHT/2.0f0
                 hit_offset = clamp((new_bp[2]-paddle_center_y_val)/(PADDLE_HEIGHT/2.0f0), -1.0f0, 1.0f0) # Float32
                 bounce_angle = hit_offset * (Float32(pi)/3.0f0) # Float32
                 cbs_val_local += BALL_SPEED_INCREASE
                 new_bv = Vec2f(cos(bounce_angle), sin(bounce_angle))*cbs_val_local # Vec2f{Float32}
            ...
            elseif new_bv[1]>0.0f0 && rect_overlaps(paddle_right_rect, ball_rect_predict)
                 ...
                 bounce_angle = hit_offset * (Float32(pi)/3.0f0) # Float32
                 cbs_val_local += BALL_SPEED_INCREASE
                 world_angle = Float32(pi) - bounce_angle # Float32
                 new_bv = Vec2f(cos(world_angle), sin(world_angle))*cbs_val_local # Vec2f{Float32}
                 new_bp = Point2f(COURT_WIDTH-PADDLE_WIDTH-BALL_SIZE/2.0f0-0.1f0, new_bp[2])
            end
            ball_pos[] = new_bp; ball_vel[] = new_bv; current_ball_speed[] = cbs_val_local # Update observable
    ...
    on(events(fig).keyboardbutton) do event
        if event.action != Keyboard.press; return Consume(false); end # Only process key presses
        ...
        if game_active[] && current_s_state != :playing
            ...
            if serve_triggered
                 angle_offset = (rand(Float32) * (Float32(pi)/3.0f0)) - (Float32(pi)/6.0f0) # Float32
                 angle = base_angle + angle_offset # Float32
                 serve_speed = current_ball_speed[]
                 ball_vel[] = Vec2f(cos(angle), sin(angle)) * serve_speed # Vec2f{Float32}
                 serve_state[] = :playing; game_message[] = ""; return Consume(true)
            end
        end
        if !game_active[] && event.key == Keyboard.r # Restart game
